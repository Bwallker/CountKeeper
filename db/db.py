...
def get_prefix(guild_id: int) -> str:
    ...
    try:
        ...
        cursor.execute(
            f"""SELECT prefix FROM prefixes WHERE guild_id = ?""", (guild_id,))
...
def get_pattern(channel_id: int) -> str:
    ...
    try:
        cursor.execute(
            f"""SELECT pattern FROM channels WHERE channel_id = ?""", (channel_id,))
...
def add_pattern(channel: ChannelClass, pattern: str) -> bool:
    ...
    try:
        ...
        cursor.execute("""INSERT INTO channels (guild_id, channel_id, pattern) VALUES(?,?,?)""",
                       (channel.guild.id, channel.id, pattern))
        channels.commit()
        successful = True
    except sqlite3.Error:
        successful = False
...
def change_pattern(channel: ChannelClass, pattern: str) -> bool:
    ...
    try:
        ...
        cursor = channels.cursor()
        cursor.execute(
            """UPDATE channels SET pattern = ? WHERE channel_id = ?""", (pattern, channel.id))
        channels.commit()
        successful = True
    except sqlite3.Error:
        successful = False
...
def get_channel_patterns(guild_id: int) -> dict[int, str]:
    ...
    try:
        cursor.execute(
            f"""SELECT channel_id FROM channels WHERE guild_id = ?""", (guild_id,))
...
def remove_notification_channel(channel_id: int) -> bool:
    ...
    try:
        ...
        cursor = notified.cursor()
        cursor.execute(
            """DELETE FROM notified WHERE channel_id = ?""", (channel_id,))
        notified.commit()
        successful = True
    except sqlite3.Error:
        successful = False
